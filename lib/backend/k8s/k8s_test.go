...
var _ = Describe("Test Syncer API for Kubernetes backend", func() {
	...
	It("should handle a CRUD of Host Endpoint", func() {
		...
		kvp1a := &model.KVPair{
			Key: model.ResourceKey{Name: kvp1Name, Kind: capiv2.KindHostEndpoint},
			Value: &capiv2.HostEndpoint{
				TypeMeta: metav1.TypeMeta{
					Kind:       capiv2.KindHostEndpoint,
					APIVersion: capiv2.GroupVersionCurrent,
				},
				ObjectMeta: metav1.ObjectMeta{
					Name: kvp1Name,
				},
				Spec: capiv2.HostEndpointSpec{
					Node:          "my-test-node1",
					InterfaceName: "eth0",
				},
			},
		...
		kvp2a := &model.KVPair{
			Key: model.ResourceKey{Name: kvp2Name, Kind: capiv2.KindHostEndpoint},
			Value: &capiv2.HostEndpoint{
				TypeMeta: metav1.TypeMeta{
					Kind:       capiv2.KindHostEndpoint,
					APIVersion: capiv2.GroupVersionCurrent,
		...
		By("Getting a missing Host Endpoint", func() {
			_, err := c.Get(ctx, model.ResourceKey{Name: "my-non-existent-test-hep", Kind: capiv2.KindHostEndpoint}, "")
			Expect(err).To(HaveOccurred())
		})

		By("Listing a missing Host Endpoint", func() {
			kvps, err := c.List(ctx, model.ResourceListOptions{Name: "my-non-existent-test-hep", Kind: capiv2.KindHostEndpoint}, "")
			Expect(err).ToNot(HaveOccurred())
			Expect(kvps.KVPairs).To(HaveLen(0))
		})

		By("Listing an explicit Host Endpoint", func() {
			kvps, err := c.List(ctx, model.ResourceListOptions{Name: kvp1Name, Kind: capiv2.KindHostEndpoint}, "")
			Expect(err).ToNot(HaveOccurred())
			Expect(kvps.KVPairs).To(HaveLen(1))
			Expect(kvps.KVPairs[0].Key).To(Equal(kvp1b.Key))
			Expect(kvps.KVPairs[0].Value.(*capiv2.HostEndpoint).ObjectMeta.Name).To(Equal(kvp1b.Value.(*capiv2.HostEndpoint).ObjectMeta.Name))
			Expect(kvps.KVPairs[0].Value.(*capiv2.HostEndpoint).Spec).To(Equal(kvp1b.Value.(*capiv2.HostEndpoint).Spec))
		})

		By("Listing all Host Endpoints", func() {
			kvps, err := c.List(ctx, model.ResourceListOptions{Kind: capiv2.KindHostEndpoint}, "")
			Expect(err).ToNot(HaveOccurred())
			...
			for _, k := range kvps.KVPairs {
				...
				vals = append(vals, k.Value.(*capiv2.HostEndpoint).Spec)
			}
			Expect(keys).To(ContainElement(kvp1b.Key))
			Expect(keys).To(ContainElement(kvp2b.Key))
			Expect(vals).To(ContainElement(kvp1b.Value.(*capiv2.HostEndpoint).Spec))
			Expect(vals).To(ContainElement(kvp2b.Value.(*capiv2.HostEndpoint).Spec))
